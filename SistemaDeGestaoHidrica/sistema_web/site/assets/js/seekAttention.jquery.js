...
jQuery.fn.seekAttention = function(o) {
    ...
    for (var i in o) {
        if (i !== 'container') {
            if (o[i] && o[i] !== 'undefined') {
                if (typeof o[i] !== typeof defaultOptions[i]) {
                    return;
                }
            }
        } else {
    ...
    function pixelateBorders(elem) {
        ...
        function makeNumerical(prop) {
            if ($(elem).css(prop).indexOf('i') > -1) {
                if ($(elem).css(prop) === 'thin') {
                    $(elem).css(prop, '2px');
                }
                if ($(elem).css(prop) === 'medium') {
                    $(elem).css(prop, '4px');
                }
                if ($(elem).css(prop) === 'thick') {
                    $(elem).css(prop, '6px');
                }
            }
        }
    ...
    $(this).each(function(i) {
        ...
        if (left < containerLeft || top < containerTop || left > containerLeft + containerWidth || top > containerTop + containerHeight) {
            return;
        }
        ...
        function attachEvents() {
            ...
            if (options.hideOnHover) {
                $('.sa-pulse-overlay').mouseover(function() {
                    ...
                    $('.sa-overlay').fadeOut();
                    $('div[class="sa-blur"]').fadeOut(400, function() {
